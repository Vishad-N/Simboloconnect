...
const Campaigns = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (results[1].status === 'fulfilled') {
                ...
                console.log("Filtered templates for UI:", approvedTemplates);
                
                // Fallback: If no approved templates found, but we have templates, show them all (for debugging/testing)
                // This prevents the "No templates" error when status might be mismatched
                if (approvedTemplates.length === 0 && allTemplates.length > 0) {
                    console.warn("No APPROVED templates found, showing ALL templates as fallback.");
            ...
            if (results[2].status === 'fulfilled') {
                const allContacts = results[2].value.data.data || results[2].value.data || [];
    ...
    const handleCreateCampaign = async (e) => {
        ...
        try {
            await axios.post(`${import.meta.env.VITE_API_URL}/api/campaigns/create`, {
                ...
                enableClickTracking: formData.enableClickTracking || false
            });
            ...
            <div className="max-w-4xl mx-auto space-y-6">
                ...
                <div className="glass-panel p-8 rounded-2xl border border-surface-700">
                    ...
                    <form onSubmit={handleCreateCampaign} className="space-y-6">
                        ...
                        <div>
                            <label className="block text-sm text-surface-300 font-medium mt-4 mb-2">Target Audience (Groups)</label>

                            {/* Retargeting banner — shows when coming from retarget action */}
                            {formData.targetPhones && formData.targetPhones.length > 0 && (
                                <div className="mb-3 p-3 bg-brand-500/10 border border-brand-500/30 rounded-xl flex items-center justify-between gap-3">
                                    <span className="text-brand-400 text-sm font-medium flex items-center gap-2">
                                        <Users size={16} /> Retargeting {formData.targetPhones.length} specific contacts
                                    </span>
                                    <button
                                        type="button"
                                        onClick={() => setFormData(prev => ({ ...prev, targetPhones: [], selectedGroups: [] }))}
                        ...
                        <div>
                            ...
                            <input
                                ...
                                className="w-full max-w-sm bg-surface-800 border-none rounded-xl px-4 py-3 text-white focus:ring-1 focus:ring-brand-500"
                            />
                            <p className="text-xs text-white0 mt-2">Leave blank to send instantly. The time is based on your local timezone.</p>
                        </div>
    ...
    if (view === 'details') {
        const calculatePercent = (val, total) => total === 0 ? 0 : Math.round((val / total) * 100);
        let totalStats = { total: 0, sent: 0, delivered: 0, read: 0, failed: 0 };
        let filteredMessages = [];

        if (selectedCampaign && selectedCampaign.messages) {
            totalStats.total = selectedCampaign.messages.length;
            totalStats.sent = selectedCampaign.messages.filter(m => ['SENT', 'DELIVERED', 'READ'].includes(m.status)).length;
            totalStats.delivered = selectedCampaign.messages.filter(m => ['DELIVERED', 'READ'].includes(m.status)).length;
            totalStats.read = selectedCampaign.messages.filter(m => m.status === 'READ').length;
            ...
            filteredMessages = selectedCampaign.messages.filter(m => {
                ...
                if (statusFilter === 'SENT') return matchesSearch && ['SENT', 'DELIVERED', 'READ'].includes(m.status);
                if (statusFilter === 'DELIVERED') return matchesSearch && ['DELIVERED', 'READ'].includes(m.status);
                                    ...
                                    <div className="flex gap-2">
                                        <button
                                            ...
                                            onClick={() => {
                                                ...
                                                setFormData({ name: `${selectedCampaign.name} - Retargeted Delivered`, templateId: '', selectedGroups: [], scheduledAt: '', enableClickTracking: false, targetPhones: recipients });
                                                setMediaUrl(''); setMediaFile(null); setVariablesConfig({});
                                                setView('create');
                                            }}
                                        ...
                                        <button
                                            ...
                                            onClick={() => {
                                                ...
                                                setFormData({ name: `${selectedCampaign.name} - Retargeted Readers`, templateId: '', selectedGroups: [], scheduledAt: '', enableClickTracking: false, targetPhones: recipients });
                                                setMediaUrl(''); setMediaFile(null); setVariablesConfig({});
                                                setView('create');
                                            }}
                                        ...
                                        <button
                                            ...
                                            onClick={() => {
                                                ...
                                                setFormData({ name: `${selectedCampaign.name} - Retry Failed`, templateId: '', selectedGroups: [], scheduledAt: '', enableClickTracking: false, targetPhones: recipients });
                                                setMediaUrl(''); setMediaFile(null); setVariablesConfig({});
                                                setView('create');
                                            }}
                                            className="px-3 py-1.5 rounded-lg bg-red-500/20 hover:bg-red-500/30 text-red-400 text-sm font-medium transition-colors border border-red-500/30 disabled:opacity-50"
                                        >
                                            Retry Failed
                                        </button>
                                    </div>

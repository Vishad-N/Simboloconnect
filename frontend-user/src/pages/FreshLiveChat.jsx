            ...
            <div className={`flex-1 min-h-0 flex flex-col bg-surface-950 ${!selectedContact ? 'hidden md:flex' : 'flex'}`}>
                {selectedContact ? (
                    <>
                        ...
                        <div className="flex-shrink-0 px-5 py-4 border-b border-white/5 bg-surface-900/80 backdrop-blur-md z-10 flex items-center justify-between shadow-sm">
                        ...
                        <div
                            ...
                            className="flex-1 min-h-0 overflow-y-auto p-6 space-y-4 relative"
                            style={{ backgroundColor: '#efeae2', backgroundImage: 'radial-gradient(circle at center, rgba(0,0,0,0.03) 1px, transparent 1px)', backgroundSize: '24px 24px', overscrollBehavior: 'contain' }}
                            ...
                            {messages.map((msg, idx) => {
                                ...
                                return (
                                    <div key={msg.id || idx} className={`flex ${isOutbound ? 'justify-end' : 'justify-start'}`}>
                                        <div
                                            className={`max-w-[70%] rounded-xl px-4 py-2 relative transition-all shadow-sm ${isOutbound
                                                ? (msg.status === 'FAILED' ? 'bg-red-50 border border-red-200 text-red-800 rounded-tr-none' : 'bg-[#d9fdd3] text-[#111b21] rounded-tr-none')
                                                : 'bg-white text-[#111b21] border border-surface-700/50 rounded-tl-none'
                                                }`}
                                        >
                        ...
                        <div className="flex-shrink-0 relative z-10 bg-[#f0f2f5]">
                            ...
                            <form onSubmit={handleSendMessage} className="flex items-center gap-3 bg-white border border-surface-700/50 p-2 rounded-[24px] shadow-sm">
                                ...
                                {isRecording ? (
                                    <div className="flex-1 flex items-center gap-3 px-2">
                                        <button type="button" onClick={cancelRecording} className="text-surface-500 hover:text-red-500 transition-colors" title="Cancel">
                                            <Trash2 size={20} />
                                        </button>
                                        <div className="w-2.5 h-2.5 rounded-full bg-red-500 animate-pulse ml-2"></div>
                                        <span className="text-red-500 font-medium text-sm w-10">{formatRecordingTime(recordingTime)}</span>
                                        <div className="flex-1 flex items-center justify-center">
                                            <canvas ref={canvasRef} width="120" height="30" className="opacity-70"></canvas>
                                        </div>
                                    </div>
                                ) : (
                                    <input ref={inputRef} type="text" value={inputText}
                                        ...
                                        className="flex-1 !bg-transparent !border-none !outline-none !shadow-none focus:!ring-0 text-[#111b21] placeholder-surface-500 text-[15px]"
                                    />
                                )}

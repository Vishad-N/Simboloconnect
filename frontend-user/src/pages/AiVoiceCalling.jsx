        ...
        <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", minHeight: '100%', paddingBottom: 40 }}>
            ...
            <div className="border-b border-white/5 bg-surface-900/30" style={{ padding: '28px 32px', marginBottom: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    ...
                    <div>
                        <h1 style={{ fontSize: 22, fontWeight: 900, color: '#ffffff', margin: 0 }}>AI Voice Calling</h1>
                        <p style={{ fontSize: 13, color: '#94a3b8', margin: '2px 0 0' }}>Automate customer calls with AI — configure providers &amp; monitor activity</p>
                    </div>
                </div>
            ...
            <div style={{ padding: '24px 32px' }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'repeat(6,1fr)', gap:14, marginBottom:28 }}>
                    {[
                        ...
                        <div key={stat.label} className="bg-surface-900 border border-white/5 rounded-2xl" style={{ padding:18, transition:'all .2s' }}
                            onMouseEnter={e=>e.currentTarget.style.transform='translateY(-2px)'}
                            onMouseLeave={e=>e.currentTarget.style.transform=''}>
                            <div style={{ fontSize:22, marginBottom:6 }}>{stat.icon}</div>
                            <div style={{ fontSize:24, fontWeight:900, color:'#ffffff' }}>{stat.value}</div>
                            <div style={{ fontSize:11, color:'#94a3b8', marginTop:3, fontWeight:600 }}>{stat.label}</div>
                        </div>
                    ))}
                </div>

                {/* Tabs */}
                <div className="bg-surface-900 border border-white/5" style={{ display:'flex', gap:4, padding:4, borderRadius:12, width:'fit-content', marginBottom:24 }}>
                    {tabs.map(tab => (
                        <button
                            key={tab.id}
                            onClick={() => setActiveTab(tab.id)}
                            className={activeTab === tab.id ? 'bg-brand-500 text-surface-950 shadow-md' : 'text-surface-400 hover:text-white hover:bg-white/5'}
                ...
                {activeTab === 'overview' && (
                    <div style={{ display:'flex', flexDirection:'column', gap:18 }}>
                        <div className="bg-surface-900 border border-white/5 rounded-2xl" style={{ padding:24 }}>
                            <h3 style={{ fontSize:16, fontWeight:800, color:'#ffffff', marginBottom:16 }}>🤖 How AI Voice Calling Works</h3>
                            <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:14 }}>
                                {[
                                    ...
                                    <div key={item.step} className="bg-surface-950 border border-white/5 rounded-xl" style={{ display:'flex', gap:14, padding:16 }}>
                                        <div style={{ width:32, height:32, borderRadius:'50%', background:'rgba(37,211,102,.15)', border:'1px solid rgba(37,211,102,.3)', display:'flex', alignItems:'center', justifyContent:'center', color:'#25D366', fontWeight:900, fontSize:14, flexShrink:0 }}>{item.step}</div>
                                        <div>
                                            <div style={{ fontSize:20, marginBottom:4 }}>{item.icon}</div>
                                            <div style={{ fontWeight:700, color:'#ffffff', fontSize:13 }}>{item.title}</div>
                        ...
                        <div className="bg-brand-500/10 border border-brand-500/20 rounded-2xl" style={{ padding:20 }}>
                            <h3 style={{ fontWeight:800, color:'#25D366', marginBottom:14, fontSize:14 }}>🛡️ Enterprise Safety Rails</h3>
                            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12 }}>
                                {['Daily Minute Limits', 'Concurrent Call Limits', 'Abuse Protection', 'Working Hours Mode'].map(feature => (
                                    <div key={feature} style={{ display:'flex', alignItems:'center', gap:8, fontSize:12, color:'#ffffff', fontWeight:600 }}>

...
const PlanActivationModal = ({ account, isOpen, onClose, closable = true }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto" style={{ background: 'rgba(0, 0, 0, 0.85)', backdropFilter: 'blur(8px)' }}>
            <div className="bg-[#0b0f19] border border-white/[0.08] text-white rounded-3xl w-full max-w-lg shadow-2xl p-6 md:p-8 relative overflow-hidden max-h-[90vh] flex flex-col">
                
                {/* Close Button */}
                {closable && (
                    <button onClick={onClose} className="absolute right-4 top-4 p-2 text-surface-400 hover:text-white rounded-full bg-white/5 transition z-10">
                        <X size={18} />
                    </button>
                )}

                {loading ? (
                    <div className="py-12 text-center text-surface-400 flex flex-col items-center justify-center gap-3">
                    ...
                    <div className="flex-1 overflow-y-auto pr-1">
                        ...
                        <div className="text-center mb-6">
                            ...
                            <h2 className="text-2xl font-black tracking-tight text-white">Activate Your Workspace</h2>
                            <p className="text-sm text-surface-400 mt-1">Please select and pay for a subscription plan to continue.</p>
                        </div>

                        {selectedPlan && (
                            <div className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-5 mb-6 relative overflow-hidden">
                                <div className="absolute top-0 right-0 bg-green-500/10 text-green-400 text-[10px] font-black tracking-widest uppercase px-3 py-1.5 rounded-bl-xl border-l border-b border-white/[0.06]">
                                    Selected Plan
                                </div>
                                <h3 className="text-lg font-extrabold text-white flex items-center gap-2">
                                    <Sparkles size={16} className="text-green-400" /> {selectedPlan.name}
                                </h3>
                                <div className="text-3xl font-black mt-2 text-white flex items-baseline gap-1">
                                    {currencySymbol}{selectedPlan.price}
                                    <span className="text-xs text-surface-400 font-medium">/{selectedPlan.duration_days} Days</span>
                                </div>

                                <ul className="mt-4 space-y-2 pt-4 border-t border-white/[0.04]">
                                    {(selectedPlan.features_json || []).map((feat, idx) => (
                                        <li key={idx} className="flex items-start text-xs text-surface-300">
                                            <Check size={14} className="text-green-400 mr-2 mt-0.5 shrink-0" />
                                            <span>{feat}</span>
                                        </li>
                                    ))}
                                </ul>
                        ...
                        <div className="mb-6">
                            <button 
                                ...
                                className="w-full text-center text-xs font-bold text-green-400 hover:text-green-300 py-2 border border-dashed border-green-500/30 rounded-xl hover:bg-green-500/5 transition flex items-center justify-center gap-1.5"
                            >
                                            ...
                                            <div 
                                                ...
                                                className={`p-3 rounded-xl border cursor-pointer transition flex items-center justify-between ${
                                                    selectedPlan?.id === p.id 
                                                        ...
                                                        : 'bg-white/[0.01] border-white/[0.06] hover:border-white/[0.12] text-surface-300'
                                                }`}
                                            >
                                                <div className="text-xs font-bold">{p.name}</div>
                                                <div className="text-xs font-black">{currencySymbol}{p.price}</div>
                                            </div>
                                        ))}
                                </div>
                            )}
                        ...
                        <div className="space-y-3 pt-2">
                            <button
                                ...
                                className="w-full py-4 rounded-xl font-bold text-black border-none text-sm transition-all hover:scale-[1.01] flex items-center justify-center gap-2"
                                style={{ background: 'linear-gradient(135deg, #25D366, #00b884)', boxShadow: '0 4px 14px rgba(37, 211, 102, 0.3)' }}
                            >
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <button
                                    ...
                                    className="flex items-center justify-center gap-1.5 py-3 rounded-xl font-bold text-white text-xs bg-white/[0.03] border border-white/[0.08] hover:bg-white/5 transition"
                                >
                                ...
                                <button
                                    ...
                                    className="flex items-center justify-center gap-1.5 py-3 rounded-xl font-bold text-white text-xs bg-white/[0.03] border border-white/[0.08] hover:bg-white/5 transition"
                                >
                                    <Phone size={14} className="text-green-400" /> Call Support
                                </button>
                            </div>
                        </div>
                    </div>
